function [x_min,x_it]=linesearch_armijo_min(f,df,hf,x0,varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(mu<=0||mu>=1||r<=0||r>=1)
        error('Parameters must be defined such that 0 < mu, r < 1');
    ...
    while(norm(df(x))>ep&&(it<=itmax))
        ...
        % Armijo condition
        while((f(x_k)>(f(x)+mu*alpha*p'*df(x)))&&alpha>ep)
            alpha = r*alpha;
            x_k=x+alpha*p;
        end
        
        % adjust x if the condition is met.
        if f(x_k)<(f(x)+mu*alpha*p'*df(x))
            x=x+alpha*p;
            x_it(it+1,:) = x';
            it=it+1;
        else 
